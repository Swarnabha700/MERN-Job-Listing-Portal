...
const MyJobs = () => {
  ...
  useEffect(() => {
    setIsLoading(true);
    fetch(`http://localhost:3000/myJobs/chakrabortyswarnabha700@gmail.com`)
      .then(res => res.json())
      .then(data => {
        setJobs(data);
        setIsLoading(false);
  ...
  return (
    <div className='max-w-screen-2xl container mx-auto xl:px-24 px-4'>
      ...
      <section className="py-1 bg-blueGray-50">
                    ...
                    <tbody>
                      {
                        currentJobs.map((job, index) => (
                          <tr key={index}>
                            ...
                            <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                              <button onClick={() => handleDelete(job._id)} className='bg-red-700 text-white py-2 px-6 rounded-md'>Delete</button>
                            </td>
                          </tr>
        ...
        {/* Pagination */}
        <div className='flex justify-center text-black space-x-8 mb-0'>
          {
            currentPage > 1 && (
